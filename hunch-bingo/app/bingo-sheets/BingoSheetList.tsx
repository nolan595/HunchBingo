...
// ─── Single sheet row ────────────────────────────────────────────────────────
function SheetItem({ sheet, index }: { sheet: SheetRow; index: number }) {
  ...
  return (
    <>
      {/* ── Desktop table row ── */}
      <tr
        className="hidden sm:table-row border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors duration-100 group animate-enter"
      ...
      {/* ── Mobile card ── */}
      <div
        className={`sm:hidden rounded-2xl border p-4 animate-enter ${
          sheet.enabled
        ...
        <>
          ...
          <div className="hidden sm:block rounded-2xl border border-slate-200 overflow-hidden bg-white shadow-sm">
            <table className="w-full text-sm">
              ...
              <tbody>
                {visible.map((sheet, i) => (
                  <SheetItem key={sheet.id} sheet={sheet} index={i} />
                ))}
              </tbody>
            </table>
          </div>
